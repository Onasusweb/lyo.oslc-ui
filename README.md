...
1. download and install jsonschema2pojo
    ...
    1. Unzip into the folder `oslc-ui\libs\`. (that is, under the path `oslc-ui\libs\jsonschema2pojo-1.0.2`, there should be `bin` & `lib` folders)
1. Go the root folder of the repo.
1. run `npm run build:elements-prod` to build the project. This will produce a `dist` with artefacts
1. Copy the `dist` folder to under `/src/main/webapp/static/` of your web project. That is, you will have a number of js/css files under the folder `/src/main/webapp/static/dist/oslc-ui`
1. Use generated `/dist/oslc-ui/java-gen/*` classes for your Java app
